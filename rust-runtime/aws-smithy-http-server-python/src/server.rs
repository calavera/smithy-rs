...
pub struct PyApp {
    ...
    workers: Arc<Mutex<Vec<PyObject>>>,
...
impl PyApp {
    async fn handle_signals(mut signals: Signals, workers: Arc<Mutex<Vec<PyObject>>>) {
        while let Some(signal) = signals.next().await {
            match signal {
                SIGTERM | SIGINT | SIGQUIT => {
                    tracing::debug!("Caught signal {signal}, shutting down all workers");
                    Python::with_gil(|py| {
                        for (idx, worker) in workers.lock().unwrap().iter().enumerate() {
                            ...
                            match worker.call_method0(py, "terminate") {
                                ...
                                Err(e) => {
                                    tracing::error!(
                                        "Error terminating worker {idx}, PID: {pid}: {e}"
                                    );
                                    if let Err(e) = worker.call_method0(py, "kill") {
                                        tracing::error!(
                                            "Error killing process {idx}, PID: {pid}: {e}"
                                        );
                                    }
                                }
                            }
                ...
                _ => tracing::debug!("Caught signal {signal}, nothing to do"),
            }
            process::exit(0);
        }
    }
...
impl PyApp {
    ...
    pub fn new(py: Python) -> PyResult<Self> {
        ...
        let handle = Some(signals.handle());
        let workers = Arc::new(Mutex::new(vec![]));
        tokio::spawn(Self::handle_signals(signals, workers.clone()));
        Ok(Self {
    ...
    pub fn register_operation(&mut self, py: Python, name: &str, func: PyObject) -> PyResult<()> {
        ...
        self.handlers.insert(String::from(name), Arc::new(handler));
        Ok(())
    }
    ...
    fn run(
        ...
        for idx in 0..workers.unwrap_or_else(num_cpus::get) {
            ...
            active_workers.push(handle.call_method0("start")?.into_py(py));
        }
        // Join (wait) all the active workers. In case of errors during `join`, the process is killed.
        for (idx, process) in active_workers.iter().enumerate() {
            let pid: isize = process
                .getattr(py, "pid")
                .map(|pid| pid.extract(py).unwrap_or(-1))
                .unwrap_or(-1);
            match process.call_method0(py, "join") {
                Ok(_) => {}
                Err(e) => {
                    tracing::error!("Error joining process {idx}, PID: {pid}: {e}");
                    if let Err(e) = process.call_method0(py, "kill") {
                        tracing::error!("Unable to kill process {idx}, PID: {pid}: {e}");
                    }
                }
            }
        }
...
impl Drop for PyApp {
    fn drop(&mut self) {
        if let Some(signal_handle) = &self.handle {
            signal_handle.close()
        }
    }
}
